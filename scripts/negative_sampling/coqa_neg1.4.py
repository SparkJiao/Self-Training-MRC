...
sample_ratio = 0.5
read_seed = 42
task_name = 'coqa-sen-r-{}-{}-{}'.format(do_negative_sampling, read_extra_self, sample_ratio)


output_dir = f"experiments/negative_sample/coqa-coqa-1.4/"
...
cmd = f"python transfer_main.py --bert_model bert-base-uncased --vocab_file {bert_base_vocab} --model_file {bert_base_model} " \
    ...
    f"--learning_rate 3e-5 --num_train_epochs 4.0 --max_answer_length 15 --max_ctx 2 " \
    f"--task_name {task_name} --bert_name hie-neg --reader_name {reader_name} --evidence_lambda 0.0 " \
    f"--sample_ratio {sample_ratio} --negative_lambda 0.8 --read_seed 42 "
