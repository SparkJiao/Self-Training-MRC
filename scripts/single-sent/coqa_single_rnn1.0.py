...
output_dir = f"experiments/coqa_single/coqa-single-rnn-1.0/"
...
cmd = f"python transfer_main.py --bert_model bert-base-uncased --vocab_file {bert_base_vocab} --model_file {bert_base_model} " \
    ...
    f"--max_seq_length 80 --max_query_length 200 --do_train --do_predict --train_batch_size 16 --predict_batch 1 " \
    f"--learning_rate 3e-5 --num_train_epochs 3.0 --max_answer_length 15 --max_ctx 2 " \
    f"--task_name {task_name} --bert_name hie-single-rnn --reader_name {reader_name} --evidence_lambda 0.8 " \
    f"--sample_ratio {sample_ratio} --negative_lambda 0.0 --extra_yesno_lambda 0.0  --gradient_accumulation_steps 16 "
