...
gen_cmd = f'python scripts/gen_evidence_by_rule.py --task_name MARCO --input_file {train_file} --top_k {2000} ' \
    f'--num_evidences {2} --output_file {root_dir}sentence_id_rule1.1-2000-2.json'
run_cmd(gen_cmd)

output_dir = f"{root_dir}v1.1/"
sentence_id_file = f"{root_dir}sentence_id_rule1.1-2000-2.json"
...
cmd = f"python main_0.6.2_topk.py --bert_model bert-base-uncased --vocab_file {bert_base_vocab} --model_file {bert_base_model} " \
    ...
    f"--do_label --sentence_id_file {sentence_id_file} --do_train --do_predict "
