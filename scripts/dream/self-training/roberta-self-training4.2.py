...
task_name = 'dream'
reader_name = 'multiple-race-roberta'
bert_name = 'topk-hie-race-roberta'
...
recurrent_times = 10
num_train_epochs = [10] + [12] * 10
num_evidence = 4
sentence_id_file = None

root_dir = f'experiments/dream/topk-evidence/self-training/roberta_v4.2_acc_top{k}'
...
for i in range(1, recurrent_times):
    ...
    cmd = f'python main2_multi_choice_top_k_evidence.py --bert_model roberta-large ' \
        ...
        f'--max_seq_length 512 --train_batch_size {1 * gradient_accumulation_steps} --predict_batch_size 1 ' \
        f'--learning_rate {learning_rate} --num_train_epochs {num_train_epochs[i]} ' \
        f'--fp16 --fp16_opt_level O2 --gradient_accumulation_steps {gradient_accumulation_steps} --per_eval_step {per_eval_step} ' \
        ...
        f'--max_grad_norm {max_grad_norm} --adam_epsilon 1e-6 --patience 8 '
