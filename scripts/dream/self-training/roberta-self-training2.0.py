...
root_dir = f'experiments/dream/topk-evidence/self-training/roberta_v2.0_acc_top{k}'
...
for i in range(recurrent_times):
    ...
    cmd = f'python main2_multi_choice_top_k_evidence.py --bert_model roberta-large ' \
        ...
        f'--max_seq_length 512 --train_batch_size 16 --predict_batch_size 1 ' \
        f'--learning_rate {learning_rate} --num_train_epochs {num_train_epochs[i]} ' \
        f'--fp16 --fp16_opt_level O2 --gradient_accumulation_steps 16 --per_eval_step 100 ' \
