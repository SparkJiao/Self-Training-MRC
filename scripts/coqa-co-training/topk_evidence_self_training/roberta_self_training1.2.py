...
top_k = 500
root_dir = f"experiments/coqa/topk-self-training/roberta_1.2_{top_k}"
...
for i in range(2, recurrent_times):
    ...
    cmd = f"python main2_0.6.2_topk.py --bert_model roberta-large " \
          ...
          f"--train_batch_size {1 * gradient_accumulation_steps} --predict_batch_size 1 --max_answer_length 15 " \
          f"--num_train_epochs {num_train_epochs[i]} --gradient_accumulation_steps {gradient_accumulation_steps} " \
          ...
          f"--max_grad_norm {max_grad_norm} --fp16 --fp16_opt_level O2 --per_eval_step 100 --adam_epsilon 1e-6 "
