...
sentence_id_file = '/home/jiaofangkai/CoQA-Challenge/BERT_RC/coqa_golden_sentence_id_file_10per.json'

top_k = 1500
root_dir = f"experiments/coqa/topk-self-training/1.0_{top_k}_10per_golden_e1.1"
...
for i in range(1, recurrent_times):
    ...
    if i == 0:
        ...
        merge_cmd = f"python general_util/full_k/merge.py " \
            ...
            f"--predict2 {origin_sentence_id_file} " \
            f"--output_dir {root_dir}/recurrent0/sentence_id_file_recurrent1.json.merge " \
            f"--label_threshold {label_threshold} --weight_threshold {weight_threshold} --k {top_k} "
        run_cmd(merge_cmd)
