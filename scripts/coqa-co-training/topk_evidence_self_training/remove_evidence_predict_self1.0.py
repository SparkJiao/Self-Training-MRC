...
for i in [0, 6]:
    ...
    cmd = f"python main_0.6.2_topk.py --bert_model bert-base-uncased --vocab_file {bert_base_vocab} --model_file {bert_base_model} " \
        ...
        f"--weight_threshold {weight_threshold} --only_correct --label_threshold {label_threshold} " \
        f"--num_evidence {num_evidence} "

    cmd += '--do_predict '
    cmd += '--remove_dict /home/jiaofangkai/CoQA-Challenge/BERT/max_f1/remove_dict.json '
    cmd += '--remove_evidence '
