...
learning_rate = 2e-5
reward_func = 1
output_dir = 'experiments/multi-rc/reinforce-fine-tune/v1.0_seed23'
...
cmd = f'python main_0.6.2_topk.py --bert_model bert-base-uncased ' \
      ...
      f'--pretrain experiments/multi-rc/gumbel-pre-train/v1.0/pytorch_model.bin'
...
cmd = f'python main_0.6.2_topk_predict_sentences.py --bert_model bert-base-uncased ' \
      ...
      f'--bert_name {bert_name} --task_name {task_name} --reader_name {reader_name} ' \
      f'--evidence_lambda {evidence_lambda} ' \
      f'--do_label --sample_steps 10 --reward_func {reward_func} ' \
      f'--num_evidence 1 '
