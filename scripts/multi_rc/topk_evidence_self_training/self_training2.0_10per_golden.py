...
sentence_id_file = '/home/jiaofangkai/CoQA-Challenge/BERT_RC/multi_rc_golden_sentence_id_file_10per.json'
num_evidence = 3

root_dir = f'experiments/multi-rc/topk-evidence/self-training/v2.0_acc_top{k}_10per_golden'
...
for i in range(recurrent_times):
    ...
    if i == 0:
        origin_sentence_id_file = f'{root_dir}/recurrent0/sentence_id_file.json'

        merge_cmd = f'python general_util/multi_rc/merge_multiple.py ' \
            f'--predict1 {sentence_id_file} ' \
            f'--predict2 {origin_sentence_id_file} ' \
            f'--output_file {root_dir}/recurrent0/sentence_id_file_recurrent1.json.merge ' \
            f'--label_threshold {label_threshold} --k {k} '
        run_cmd(merge_cmd)
