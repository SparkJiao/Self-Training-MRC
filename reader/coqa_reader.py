...
from data.data_instance import SQuADFullExample, SQuADFullInputFeatures, RawResultChoice, WeightResultChoice, WeightResult, \
    QAFullInputFeatures, QAFullExample, ModelState
...
class CoQAYesNoReader(object):
    ...
    def read(self, input_file, dialog_turns: int = 2, remove_evidence=False, remove_passage=False, remove_question=False,
             remove_dict=None):
        """
        ...
        logger.info('Remove evidence during test: {}'.format(remove_dict))
        ...
        for paragraph in input_data:
            ...
            for i, (question, answer) in enumerate(zip(questions, answers)):
                ...
                rationale_start_position = char_to_word_offset[answer['span_start']] if not remove_passage else 0
                rationale_end_position = char_to_word_offset[answer['span_end'] - 1] if not remove_passage else 0
                ...
                example = SQuADFullExample(
                    qas_id=qas_id,
                    question_text=question_text if not remove_question else '',
    ...
    def write_sentence_predictions(all_examples, all_features: List[QAFullInputFeatures], all_results: List[WeightResult],
        ...
        return {'predictions': all_predictions, 'yesno_acc': yesno_pred.avg, 'sentence_acc': sentence_pred.avg}
    ...
    @staticmethod
    def generate_inputs(batch: Tuple, all_features: List[SQuADFullInputFeatures], model_state, do_label: bool = False):
