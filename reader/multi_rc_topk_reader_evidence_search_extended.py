...
"""
Reader to search evidence sentence during inference.
"""


class MultiRCTopKReader(object):
    def __init__(self):
        super(MultiRCTopKReader, self).__init__()
    ...
    def read(self, input_file, remove_evidence=False, remove_question=False, remove_passage=False, remove_dict=None,
        ...
        logger.info(f'Evidence searched file: {evidence_search_file}')
        with open(input_file, "r", encoding='utf-8') as reader:
            input_data = json.load(reader)

        if evidence_search_file is not None:
            searched_evidences = json.load(open(evidence_search_file, 'r'))
        else:
            searched_evidences = None
        ...
        for item in tqdm(input_data):
            ...
            for sent_id, sent in enumerate(article):
                ...
                # for pq_id in searched_evidences:
                #     assert len(searched_evidences[pq_id]) == 1, (pq_id, searched_evidences[pq_id])

                instance = copy.deepcopy(item)
                instance['article'] = [sent]
                ...
                for q_id, (question, option_list, option_answer_list, evidence) in enumerate(zip(questions, options, answers, evidences)):
                    ...
                    for op_id, (option, answer) in enumerate(zip(option_list, option_answer_list)):
                        ...
                        if searched_evidences is not None:
                            search_id = f'{instance["id"]}--{q_id}--{op_id}'
                            searched_sentences = searched_evidences[search_id][:]

                            if sent_id in searched_sentences:
                                continue
                            searched_sentences.append(sent_id)

                            searched_sentences = sorted(searched_sentences, reverse=False)

                            instance['article'] = [article[s_id] for s_id in searched_sentences]
                            # assert len(instance['article']) == 2, (searched_sentences, searched_evidences[search_id])
                            new_id = '#' + '#'.join([str(x) for x in searched_sentences])
                        else:
                            new_id = f'#{sent_id}'
                        ...
                        qas_id = f"{article_id}{new_id}--{q_id}--{op_id}"
    ...
    @staticmethod
    def convert_examples_to_features(examples: List[MultiRCExample], tokenizer, max_seq_length: int = 512, **kwargs):
    ...
    def write_sentence_predictions(self, all_examples, all_features: List[MultiRCFeature], all_results: List[RawOutput],
        ...
        for (example_index, example) in enumerate(all_examples):
            feature = example_index_to_features[example_index][0]
            result = unique_id_to_result[feature.unique_id]
            choice_logits = np.array(result.model_output["choice_logits"][1:])
            choice_logits = choice_logits / choice_logits.sum()  # Recalculate probability after remove index 0.
            prediction = int(choice_logits.argmax())

            sentence_logits = result.model_output["sentence_logits"]
            sentence_ids = result.model_output["sentence_ids"]
            gold_sentence_ids = example.sentence_ids
            ...
            all_predictions[example.qas_id] = {
                ...
                'max_sentences': len(example.sentence_span_list)
            }
            if example.answer is not None:
                all_predictions[example.qas_id]['answer'] = example.answer - 1
    ...
    def generate_features_sentence_ids(features: List[MultiRCFeature], sentence_label_file):
        ...
        for feature_id, feature in enumerate(features):
            ...
            if qas_id in sentence_label_dict:
                ...
                #         labeled_data += 1
                masked_sentence_ids = [sent_id for sent_id in sentence_label_dict[qas_id]['sentence_ids'] if
                                       sent_id < len(feature.sentence_span_list)]

                sentence_ids = masked_sentence_ids
                if sentence_ids:
                    labeled_data += 1
    ...
    def data_to_tensors_sent_pretrain(self, all_features: List[MultiRCFeature]):
        all_features = [feature for feature in all_features if feature.sentence_ids]
