...
class MultiRCTopKReader(object):
    ...
    def read(self, input_file, remove_evidence=False):
    ...
    @staticmethod
    def convert_examples_to_features(examples: List[MultiRCExample], tokenizer: RobertaTokenizer, max_seq_length: int = 512, **kwargs):
        ...
        for (example_index, example) in tqdm(enumerate(examples), desc='Convert examples to features', total=len(examples)):
            ...
            utils.truncate_seq_pair(q_op_tokens, doc_tokens, max_seq_length - 4)

            tokens = [tokenizer.cls_token] + q_op_tokens + [tokenizer.sep_token, tokenizer.sep_token]
