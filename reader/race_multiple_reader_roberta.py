...
class RACEMultipleReader(object):
    ...
    @staticmethod
    def convert_examples_to_features(examples: List[MultiChoiceFullExample], tokenizer: RobertaTokenizer, max_seq_length: int = 512):
        ...
        for (example_index, example) in tqdm(enumerate(examples), desc='Convert examples to features', total=len(examples)):
            ...
            for option_id, option in enumerate(options):
                ...
                inputs = tokenizer.encode_plus(q_op_tokens, doc_tokens, add_special_tokens=True, max_length=max_seq_length)
                input_ids, segment_ids = inputs["input_ids"], inputs["token_type_ids"]
                input_mask = [1] * len(input_ids)

                truncate_doc_len = sum(segment_ids) - 1
                truncate_q_op_len = len(segment_ids) - sum(segment_ids) - 3
                doc_offset = truncate_q_op_len + 3  # <s> A </s></s> B </s>

                sentence_list = []
                # doc_offset = len(q_op_tokens) + 2
                for (start, end) in sentence_spans:
                    assert start <= end
                    if start >= truncate_doc_len:
                        break
                    if end >= truncate_doc_len:
                        end = truncate_doc_len - 1
