...
def main():
    ...
    parser.add_argument('--dev_sentence_id_file', default=None, type=str)
    parser.add_argument('--freeze_predictor', default=False, action='store_true')
    ...
    if args.do_train or args.do_label:
        ...
        train_features, train_tensors = data_reader.data_to_tensors_sent_pretrain(train_features)
        num_train_steps = int(
            train_tensors[0].size(0) / args.train_batch_size / args.gradient_accumulation_steps * args.num_train_epochs)
    ...
    eval_features = data_reader.convert_examples_to_features(examples=eval_examples, tokenizer=tokenizer,
                                                             max_seq_length=args.max_seq_length, doc_stride=args.doc_stride,
                                                             max_query_length=args.max_query_length)
    if args.dev_sentence_id_file:
        eval_features = data_reader.generate_features_sentence_ids(eval_features, args.dev_sentence_id_file)
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", dynamic_ncols=True)):
                ...
                if (step + 1) % args.per_eval_step == 0 or step == len(train_dataloader) - 1:
                    ...
                    for _, eval_batch in enumerate(tqdm(eval_dataloader, desc="Evaluating", dynamic_ncols=True)):
                        ...
                        inputs = data_reader.generate_inputs(eval_batch, eval_features, model_state=ModelState.Train)
                        with torch.no_grad():
                            output_dict = model(**inputs)
                            loss = output_dict['loss']
                    ...
                    logger.info(f"Sentence loss: {eval_epoch_loss}")
                    ...
                    logger.info('Eval Epoch: %d, sentence loss: %f (Best loss: %f)' % (eval_epoch, eval_epoch_loss, best_loss))
                    eval_epoch += 1
