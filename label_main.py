...
from data.data_instance import RawResultChoice, WeightResultChoice, ModelState, ReadState, FullResult
...
def main():
    ...
    for feature in train_features:
        if feature.sentence_id == -1:
            no_evidence += 1
    logger.info(f'No evidence ratio: {no_evidence} / {len(train_features)} = {no_evidence * 1.0 / len(train_features)}')
    ...
    if args.do_train:
        ...
        if args.sentence_id_file is not None:
            ...
            data_reader.generate_features_sentence_ids(train_features, args.sentence_id_file)
        else:
