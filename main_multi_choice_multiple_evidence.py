...
def main():
    ...
    if args.do_label and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        def batch_choice_beam_search(sentence_sim, sentence_mask, beam_num=10) -> List[List[Dict]]:
            """
            ...
            for b in range(batch_size):
                ...
                for c in range(num_choices):
                    choice_sentence_ids.append(beam_search(sentence_sim[b, c, :int(sum(sentence_mask[b, c]))], beam_num))
                    print('=================single choice=====================', file=f, flush=True)
