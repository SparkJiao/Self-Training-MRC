            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", dynamic_ncols=True)):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if global_step % args.per_eval_step == 0:
                        ...
                        summary_writer.add_scalar('eval_acc', current_acc, global_step)
                        torch.cuda.empty_cache()

                        if args.save_all:
                            model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
                            output_model_file = os.path.join(args.output_dir, f"checkpoint-{global_step}.bin")
                        ...
                        logger.info('Global Step: %d, Accuracy: %.4f (Best Accuracy: %.4f)' % (global_step, current_acc, best_acc))
                        eval_epoch += 1
