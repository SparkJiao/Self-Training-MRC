...
from data.data_instance import RawResultChoice, WeightResultChoice, ModelState, WeightResult
...
def main():
    ...
    #             inputs = data_reader.generate_inputs(batch, train_features, do_label=args.do_label, model_state=ModelState.Train)
    ...
    #         for eval_step, batch in enumerate(tqdm(eval_dataloader, desc="Evaluating", dynamic_ncols=True)):
    #             if n_gpu == 1:
    #                 batch = batch_to_device(batch, device)  # multi-gpu does scattering it-self
    #             inputs = data_reader.generate_inputs(batch, eval_features, do_label=args.do_label, model_state=ModelState.Evaluate)
    #             with torch.no_grad():
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for batch in tqdm(test_dataloader, desc="Testing", dynamic_ncols=True):
            if n_gpu == 1:
                batch = batch_to_device(batch, device)  # multi-gpu does scattering it-self
            inputs = data_reader.generate_inputs(batch, test_features, do_label=args.do_label, model_state=ModelState.Test)
            with torch.no_grad():
                output_dict = model(**inputs)
        ...
        output = data_reader.write_sentence_predictions(eval_examples, eval_features, all_results,
